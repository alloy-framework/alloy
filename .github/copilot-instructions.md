...
**Validation workflow tip:** use `pnpm --filter @alloy-js/rust build && pnpm --filter @alloy-js/rust test` as the Rust package correctness gate; treat `pnpm --filter rust-example build` `TS2742` in `samples/rust-example/src/externals.ts` as a separate sample bug to track.
