...
**Fix approach:** Use explicit type annotations on the exported `std` constant rather than relying on type inference, per the custom instruction: *"Prefer explicit exported type aliases/interfaces for the export surface."*
