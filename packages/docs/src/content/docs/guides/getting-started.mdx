...
> 💡 The build defaults to production (no JSX source info). Use `--dev` to include file/line/column info, or `--no-source-info` to force-disable it. `BABEL_ENV`/`NODE_ENV` can also control this (dev if not "production").
